...
8. citas-api/docs/FCV Dev/llm-wiki/wiki/index.md si existe
9. citas-api/docs/FCV Dev/subagents/README.md si existe
...
- La Skill scrum-spec-orchestrator mantiene una allowlist propia; si todavía apunta a `citas-api/docs/wiki/scrum/`, no la ejecutes sobre `docs/FCV Dev/scrum/` hasta que esa Skill sea actualizada explícitamente.
...
SUBAGENTES
El catálogo canónico vive en `citas-api/docs/FCV Dev/subagents/README.md` y contiene ocho perfiles:
- `subagents/backend/01-domain-hexagonal.md`: invariantes, casos de uso y puertos;
- `subagents/backend/02-persistence-mysql.md`: JPA, Flyway, 3FN y concurrencia;
- `subagents/backend/03-security-jwt.md`: autenticación, autorización y JWT;
- `subagents/backend/04-backend-verifier.md`: CA/DoD backend, siempre solo lectura;
- `subagents/frontend/01-design-reconciler.md`: fidelidad al diseño aprobado;
- `subagents/frontend/02-api-integration.md`: REST, DTO y ciclo de sesión;
- `subagents/frontend/03-state-forms.md`: formularios, estados y accesibilidad;
- `subagents/frontend/04-frontend-verifier.md`: CA/DoD frontend, siempre solo lectura.

Selecciona el perfil mínimo que cubra la tarea. Cada encargo debe indicar objetivo, HU/CA/DoD, repositorio/rama, archivos permitidos, contrato o diseño de referencia, autoridad de edición, pruebas/evidencia y fuera de alcance. No asignes el mismo archivo a dos subagentes con permiso de edición. Los verificadores nunca implementan y se ejecutan después del cambio como revisión independiente. Si un hallazgo implica modificar un contrato REST, vuelve al flujo cross-repo antes de editar.
...
- RAW: `citas-api/docs/FCV Dev/llm-wiki/raw/` contiene fuentes curadas/inmutables (PRD aprobado, decisiones, contratos, notas validadas). El agente lee, no reescribe fuentes durante ingest.
